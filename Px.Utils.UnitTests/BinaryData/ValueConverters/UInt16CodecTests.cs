    ...
    public class UInt16CodecTests
    {
        ...
        public void WriteReadExistsRoundtrip()
        {
            ...
            DoubleDataValue[] input = [
                ...
                new DoubleDataValue(65528, DataValueType.Exists) // Adjusted to avoid sentinel values
            ];
        ...
        public void ReadDecimalExistsRoundtrip()
        {
            ...
            DoubleDataValue[] input = [
                new DoubleDataValue(0, DataValueType.Exists),
                new DoubleDataValue(123, DataValueType.Exists),
                new DoubleDataValue(65527, DataValueType.Exists)
            ];
            codec.Write(input, ms);
            ReadOnlySpan<byte> bytes = ms.ToArray();
            codec.Read(bytes, output);
            ...
            Assert.AreEqual(123m, output[1].UnsafeValue);
            Assert.AreEqual(DataValueType.Exists, output[2].Type);
            Assert.AreEqual(65527m, output[2].UnsafeValue);
        }
        ...
        public void BoundaryValuesAndSentinels()
        {
            ...
            using MemoryStream ms = new();
            DoubleDataValue[] input = [
                new DoubleDataValue(65527, DataValueType.Exists), // just below sentinel start
                new DoubleDataValue(UInt16Codec.SentinelStart, DataValueType.Exists), // sentinel value
                new DoubleDataValue(UInt16Codec.SentinelStart + 1, DataValueType.Exists) // just above sentinel start
            ];
            codec.Write(input, ms);
            ReadOnlySpan<byte> bytes = ms.ToArray();
            DoubleDataValue[] output = new DoubleDataValue[input.Length];
            codec.Read(bytes, output);
            Assert.AreEqual(DataValueType.Exists, output[0].Type);
            Assert.AreEqual(65527, output[0].UnsafeValue);
